...
int main(int argc, char ** argv) {
  ...
  for (int ep = 0; ep < E; ep++) {
    ...
    // BEGIN ANSWER: サンプルのループを配列 reduction で並列化せよ: #pragma omp parallel for reduction(+:loss,correct,gb2,gW1[:H*D],gb1[:H],gW2[:H]).
#pragma omp parallel for reduction(+:loss,correct,gb2,gW1[:H*D],gb1[:H],gW2[:H])
    // END ANSWER
    for (long i = 0; i < N; i++) {
