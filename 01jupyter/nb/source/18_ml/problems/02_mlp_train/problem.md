...
**勾配は「行列積」で求まる** (`gW2 = dO Hᵀ` など) のがポイント。サンプル(バッチ)方向の和は**行列積の内側の縮約**に入るので, 並列化は各 forward/backward の**独立な出力方向のループを並列化するだけ**でよく, 勾配への**配列 reduction は要らない** (損失・正解数のスカラ集計にだけ `reduction` を使う):

- `forward`: 各サンプル(列) `i` は独立 → サンプルのループを `#pragma omp parallel for`(Fortran は `!$omp parallel do`)で並列化。損失・正解数だけ `reduction(+:loss,correct)`。
- `backward`: `dO`・`dH` は列 `i` ごと, `gW2,gb2` は出力 `c` ごと, `gW1,gb1` は隠れ `k` ごとに独立 → それぞれの外側ループを `parallel for` / `parallel do` で並列化 (reduction 不要)。

`forward` と `backward` はこれらのバッチ行列演算 (`dense_relu`, `dense_softmax`, `out_grad`, `grad_weight`, `back_relu`) を呼ぶだけ。各演算の中のループ先頭が `TODO` になっている。スレッド数を変えても結果 (正解率) は同じになることを確認せよ。
