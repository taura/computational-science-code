...
訓練データ (NumPy 標準の **`.npy` 形式**: ヘッダ + 生バイナリ) は Wisteria の共有領域に置いてある。冒頭の「データの準備」セルで張った symlink `data/` 経由で読み, 学習した重みは自分のフォルダの `weights/` に書き出す (世の中で配布されている MNIST (`mnist.npz`) から取り出した訓練画像):

- `data/x_train.npy` : MNIST 訓練画像 (各 784 画素, uint8 で 0..255)
- `data/y_train.npy` : 正解ラベル (int32, 0..9)
- 学習後, `weights/{W1,b1,W2,b2}.npy` に重みを保存する (推論 (18_ml/problems/00_mnist_infer) がこれを読む)
