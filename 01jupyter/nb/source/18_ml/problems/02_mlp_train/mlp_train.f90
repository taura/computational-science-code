...
program mlp_train
  ...
  do ep = 0, E - 1
     ...
     ! BEGIN ANSWER: サンプルのループを配列 reduction で並列化せよ: !$omp parallel do private(...) reduction(+:loss,correct,gb2,gW1,gb1,gW2) (h は private)。
     !$omp parallel do private(i,k,x0,x1,o_in,o,yi,dout,dh,z,hk,hh) &
     !$omp   reduction(+:loss,correct,gb2,gW1,gb1,gW2)
     ! END ANSWER
     do i = 0, N8 - 1
        allocate(hh(0:H-1))
