...
板の温度 `u(x,y)` が**定常** (時間変化しない) のとき, 熱伝導方程式は **ラプラス方程式**

```
∂²u/∂x² + ∂²u/∂y² = 0       (∇²u = 0)
```

になる。`u` は温度, 板の縁 (境界) の温度は与えた値に固定する (上端 100, 他 3 辺 0)。

格子間隔 `h` で離散化する。2階微分を**中心差分**で近似すると
...
これを足して `= 0` と置き `u(i,j)` について解くと, `h²` が消えて

```
u(i,j) = ( u(i−1,j) + u(i+1,j) + u(i,j−1) + u(i,j+1) ) / 4
```

すなわち **「各点の温度 = 上下左右4点の平均」** (5点ステンシル)。この関係が満たされるまで, 右辺の平均で `u(i,j)` を繰り返し置き換えるのが**ヤコビ反復**である。
