...
2D熱伝導 (15_pde_fdm/problems/00_heat2d)・2D波動 (15_pde_fdm/problems/01_wave) と同じ stencil の構造で, 空間の二重ループを `collapse(2)` で並列化する。**反射境界で総量が保存される**ことを検算に使う。

## 背景: 元の方程式と差分化

インクの濃度 `u(x,y,t)` は **拡散方程式**

```
∂u/∂t = D ( ∂²u/∂x² + ∂²u/∂y² )
```

に従う。`u` は濃度, `D` は拡散係数 (大きいほど速く広がる)。

時間は1階の**前進差分**, 空間は2階の**中心差分**で離散化する (この組み合わせを **FTCS**: Forward-Time, Central-Space という):
...
`u^{n+1}` について解くと, `alpha = D·Δt/Δx²` として

```
u^{n+1} = u^n + alpha × (上 + 下 + 左 + 右 − 4×中央)
```

現在の値だけから未来が直接求まる陽解法。`alpha ≤ 0.25` が安定条件 (これを超えると振動・発散する)。
...
- 並列化するのは**空間ループ** (各時間ステップ内)。時間ループは逐次 (2D波動 (15_pde_fdm/problems/01_wave) と同じ)。
- 各点の更新は独立で総和も無いので reduction は不要 (2D熱伝導 (15_pde_fdm/problems/00_heat2d) の残差判定とは違う)。
