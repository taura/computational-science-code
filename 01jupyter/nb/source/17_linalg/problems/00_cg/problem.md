...
`A x = b` の `A` は, 熱伝導 (B1) と同じ **2次元ラプラシアン** である。これは **ポアソン方程式**
...
B1 (熱伝導) ではヤコビ反復で少しずつ平均を取って解いたが, 同じ方程式を **連立一次方程式 `A x = b`** とみなして反復解法で直接解くこともできる。それが CG 法である (`A` は対称正定値 SPD なので CG が使える)。
